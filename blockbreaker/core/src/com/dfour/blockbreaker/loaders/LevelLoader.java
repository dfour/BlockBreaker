...
public class LevelLoader {
	...
	public void loadLevel(int level){
		...
		for (int y = 0; y < 30; y++) { // for each line of 30 lines
			for (int x = 0; x < 40; x++) { // for each row of 40 chars
				char coord = text.charAt((y * 40) + x); // current line * 40 chars per line + amount of chars in
				if (coord == 'x') {
					model.entFactory.makeBrick(x * 2, (Math.abs(y - 30)) * 2); // each brick is 2 high and 2 wide
				} else if (coord == 's') {
					model.entFactory.makeLight(x * 2, (Math.abs(y - 30)) * 2);
				}else if (coord == '/') {
					model.entFactory.makeStaticObstacle(x * 2, (Math.abs(y - 30)) * 2,false);
				}else if (coord == '\\') {
					model.entFactory.makeStaticObstacle(x * 2, (Math.abs(y - 30)) * 2, true);
				}else if (coord == 'c') {
